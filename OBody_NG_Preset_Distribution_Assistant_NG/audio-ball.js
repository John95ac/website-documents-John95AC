(function() {
  ...
  function createElements() {
    ...
    ball.id = 'audio-ball';
    ball.innerHTML = '🎵';
    ball.title = 'Selección de música';
    ...
    controls.innerHTML = `
      ...
      <br>
      <label for="volumeSlider" style="font-size: 12px;">Volumen:</label>
      <input type="range" id="volumeSlider" min="0" max="100" value="${targetVolume}" style="width: 100%; margin: 5px 0;">
      <br>
      <a href="#" id="audio-picker" title="Elegir pista" style="color: white; margin-right: 5px;">Lista</a>
    ...
    function setIcon(isPlaying) {
      ...
      $toggle.title = isPlaying ? 'Mutear' : 'Reproducir';
    }
      ...
      player = new YT.Player($container, {
        ...
        events: {
          onReady: (ev) => {
            ...
            if (desiredState === 'paused') ev.target.pauseVideo();
            else tryPlay();
            updateTitle($title);
    ...
    $toggle.onclick = (e) => {
      ...
      if (state !== YT.PlayerState.PLAYING) {
        ...
        localStorage.setItem(storageKey, desiredState);
      } else {
        player.pauseVideo();
        setIcon(false);
        desiredState = 'paused';
