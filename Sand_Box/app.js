...
function initializeCopyFunction() {
    ...
    if (copyRuleBtn && generatedRule) {
     ...
     // Initialize download buttons for simulator
     const downloadJSONBtn = document.getElementById('downloadJSONBtn');
     const downloadJSBtn = document.getElementById('downloadJSBtn');
     const downloadZIPBtn = document.getElementById('downloadZIPBtn');
     if (downloadJSONBtn && downloadJSBtn && downloadZIPBtn && generatedRule) {
         ...
         downloadJSONBtn.addEventListener('click', function() {
             ...
             // If there's a preview (ends with comment + rule), use all; else just permanent
             const lines = ruleText.split('\n');
             if (lines.length >= 2 && lines[lines.length - 2].startsWith(';') && lines[lines.length - 1].includes('=')) {
                 ruleText = ruleText;
             } else {
             ...
             if (!ruleText) {
                 showCopyError(this, 'No rules to download');
                 return;
             }

             // Convert INI text to JSON object
             const jsonObject = {
                 rules: ruleText,
                 timestamp: new Date().toISOString(),
                 format: 'OBody NG PDA INI Rules'
             };
             
             const jsonContent = JSON.stringify(jsonObject, null, 2);
             downloadFile(jsonContent, 'rules.json', this, 'application/json;charset=utf-8');
         });
         

         
         // Download as ZIP (containing JSON)
         downloadZIPBtn.addEventListener('click', function() {
             ...
             // Create ZIP with JSON file inside
             const zip = new JSZip();
             const jsonObject = {
                 rules: ruleText,
                 timestamp: new Date().toISOString(),
             ...
             const jsonContent = JSON.stringify(jsonObject, null, 2);
             zip.file("rules.json", jsonContent);
      ...
      const downloadZIPIniBtn = document.getElementById('downloadZIPIniBtn');
      if (downloadZIPIniBtn && generatedRule) {
         downloadZIPIniBtn.addEventListener('click', function() {
             console.log('Download ZIP with INI button clicked');
             let ruleText = generatedRule.innerText.trim();
...
/**
 * Download file with specified content type
 */
function downloadFile(content, fileName, button, mimeType = 'text/plain;charset=utf-8') {
    try {
